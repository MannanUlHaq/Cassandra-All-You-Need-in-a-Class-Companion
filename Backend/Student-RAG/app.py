...
def supabase_insert(table: str, data: Dict[str, Any]):
    ...
    response.raise_for_status()
    # Post requests don't always return JSON when inserting, sometimes it's empty
    try:
        return response.json()
    except Exception:
        return True
...
def generate_and_store_notes_vectors(text: str, course_id: str, note_id: str):
    """
    Specifically handles direct raw text (markdown notes) sent from the Notes Editor
    and stores them in the notes_material table.
    """
    chunks = smart_chunk_text(text, CHUNK_SIZE, CHUNK_OVERLAP)
    if not chunks:
        print("⚠️ No valid text found to chunk for notes.")
        return
        
    print(f"📥 Generating embeddings for {len(chunks)} note chunks...")
    batch_size = 4
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        for chunk_idx, (chunk_text, embedding) in enumerate(zip(batch_texts, embeddings_list)):
            metadata = {
                "note_id": note_id,
            ...
            # Targeting the `notes_material` table as per the schema requested
            supabase_insert('notes_material', {
                'content': chunk_text,
                'metadata': metadata,
                'embedding': embedding.tolist(),
                'course_id': course_id
        ...
        del embeddings, embeddings_list, batch_texts
        gc.collect()
...
# Existing endpoint for physical files (PDF/DOCX)
...
# New Endpoint for Raw Notes Vectorization
...
def upload_notes():
    try:
        ...
        teacher_id = data.get('teacher_id') # For validation/future use

        if not text or not course_id or not note_id:
            return jsonify({'error': 'Missing text, course_id, or note_id parameter.'}), 400

        # Vectorize and push directly to `notes_material` table
        generate_and_store_notes_vectors(text, course_id, note_id)

        return jsonify({'success': True, 'message': 'Notes vectorized and stored successfully.'}), 200
