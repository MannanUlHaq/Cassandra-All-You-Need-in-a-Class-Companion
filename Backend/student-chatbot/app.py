...
# Retrieve secrets from Azure App Service Environment Variables
...
def get_chat_history(student_id: str, course_id: str, limit: int = 5):
    """Get last N messages from chat history for a specific course for the student"""
    require_supabase()
    url = f"{SUPABASE_REST_URL}/student_chat_history"
    params = {
...
def save_chat_message(student_id: str, course_id: str, message: str, response: str):
    """Save chat message and response for a specific course"""
    require_supabase()
    import uuid
    url = f"{SUPABASE_REST_URL}/student_chat_history"
    data = {
        "student_chat_id": str(uuid.uuid4()),  
...
def search_similar_content(query: str, course_id: str, top_k: int = 4):
    """Search notes_material with automatic fallback to exact string matching"""
    try:
        with torch.no_grad():
        ...
        # Make sure you create this RPC function in Supabase targeting `notes_material`
        results = supabase_rpc("search_similar_notes", {
...
# ---------------------------
SYSTEM_PROMPT = """You are Cassandra, a supportive and intelligent AI teaching assistant for students. 
Your goal is to help students understand their course concepts deeply based strictly on the provided course notes and previous conversation history.

**Guidelines:**
- Act as a tutor: guide the student to understanding rather than just doing their work for them.
- Explain concepts clearly, encouragingly, and in easy-to-understand language.
- Base your factual answers heavily ON THE PROVIDED COURSE NOTES.
- Reference the conversation history when relevant to maintain context.
- If the provided notes don't contain the answer, politely inform the student that the information isn't explicitly covered in the current notes, but offer a brief general explanation if appropriate.
- Keep your responses structured, readable, and directly address the student's core question."""
...
def build_context_with_history(retrieved_docs: List[Dict], chat_history: List[Dict]) -> str:
    ...
    if chat_history:
        ...
        for entry in chat_history:
            context_parts.append(f"Student: {entry['message']}")
            context_parts.append(f"Cassandra: {entry['response']}")
        context_parts.append("")
    
    if retrieved_docs:
        context_parts.append("**Relevant Course Notes:**")
...
def generate_answer(question: str, context: str) -> str:
    ...
    payload = {
        ...
        "messages": messages,
        "max_tokens": 500,
        "temperature": 0.5,  # Slightly higher for more conversational/tutoring tone
        "top_p": 0.9
    }
...
def chat():
    try:
        ...
        if not message or not student_id or not course_id:
            return jsonify({'error': 'Message, student_id, and course_id are required fields'}), 400
        
        chat_history = get_chat_history(student_id, course_id, limit=5)
        ...
        save_chat_message(student_id, course_id, message, answer)
...
def get_history():
    try:
        ...
        history = get_chat_history(student_id, course_id, limit)
